...
class ConnectService {
  ...
  private static getMessagesCacheKey(
    userId: string,
    receiverId: string,
  ): string {
  ...
  static async getMessages(
    ...
    // Store in cache
    await redisClient.set(cacheKey, JSON.stringify({ messages, total }), {
      EX: this.MESSAGE_CACHE_EXPIRATION,
    });
  ...
  static async getRecentConversations(userId: string) {
    ...
    // Store in cache
    await redisClient.set(cacheKey, JSON.stringify(refinedConversations), {
      EX: this.CONVERSATION_CACHE_EXPIRATION,
    });
  ...
  static async deleteConversation(userId: string, recipientId: string) {
    ...
    try {
      // Delete all messages between the user and recipient
      ...
      // Invalidate cache
      await invalidateCache(this.getMessagesCacheKey(userId, recipientId));
      await invalidateCache(this.getMessagesCacheKey(recipientId, userId));
      await invalidateCache(this.getConversationsCacheKey(userId));
      await invalidateCache(this.getConversationsCacheKey(recipientId));
