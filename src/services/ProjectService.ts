...
import {invalidateProjectInteractionCache} from "../redis/cacheUtils";
...
class ProjectService {
    ...
    private getPublishedProjectsKey(): string {
    ...
    async getProjectById(projectId: string) {
        ...
        if (!project || !project.status) throw new Error("Project not found");
    ...
    async getPublishedProjects(limit = 10) {
        const cacheKey = this.getPublishedProjectsKey();
...
// only cache if meaningful
    ...
    async deleteProject(projectId: string) {
        ...
        await invalidateProjectInteractionCache(projectId, userId);
